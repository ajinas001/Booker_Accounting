...
const services = [
  ...
  },
  {
    ...
    buttonText: 'Read Our Latest Report',
    bgColor: 'bg-black',
    textColor: 'text-white',
    buttonBorder: 'border-white',
    type: 'content',
  ...
  },
{
  ...
  description:
    'We help you navigate the complexities of UAE taxation with expert corporate tax, VAT, and transfer pricing advisory for seamless compliance and growth.',
  buttonText: 'Check Our Solutions',
  bgColor: 'bg-secondary',
  textColor: 'text-white',
  buttonBorder: 'border-white',
  type: 'content',
  // make it large (2 columns wide, 1 row tall)
  span: 'lg:col-span-2 lg:row-span-1',
...
},
{
  ...
  description:
    'We help businesses implement strong AML controls through risk assessment, compliance training, and policy development for full UAE regulatory adherence.',
  buttonText: 'Get Started',
  bgColor: 'bg-secondary',
  textColor: 'text-white',
  buttonBorder: 'border-white',
  type: 'content',
  // make it smaller (1 column wide, 1 row tall)
  span: 'lg:col-span-1 lg:row-span-1',
  borderRadius: 'rounded-[45px]',
  height: 'min-h-[360px]',
},
  ...
  {
    id: 9,
    title: 'Company Formation & Liquidation',
    description: 'Use this space to promote the business, its products or its services.',
...
const ArrowIcon = () => (
  <svg width="60" height="80" viewBox="0 0 60 80" fill="none" className="mx-auto">
    <path d="M30 0 L30 60 M30 60 L15 45 M30 60 L45 45" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
          ...
          {services.map((item) => {
            if (item.type === 'icon') {
              return (
                ...
                >
                  <div className={item.iconType === 'arrow' ? 'text-black' : 'text-black'}>
                    {item.iconType === 'arrow' ? <ArrowIcon /> : <StarburstIcon />}
                  </div>
                </div>
              );
            }
            ...
            return (
              <div
                ...
                className={`${item.bgColor} ${item.borderRadius} ${item.span} ${item.height} overflow-hidden`}
              >
                <div className={`p-8 md:p-10 lg:p-12 flex flex-col justify-between h-full ${item.textColor}`}>
                  <div>
                  ...
                  <button className={`w-fit px-8 py-3 rounded-tl-full rounded-tr-full rounded-br-full border-2 ${item.buttonBorder} ${item.textColor} text-sm font-light hover:opacity-80 transition-opacity`}>
                    {item.buttonText}
                  </button>
                </div>
              </div>
