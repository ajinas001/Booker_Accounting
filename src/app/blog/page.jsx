...
const CategoryTag = ({ text }) => {
  ...
  const buttonStyle = "px-8 py-3 text-sm font-semibold tracking-wider uppercase text-white bg-gray-800 transition duration-300 hover:bg-gray-700";
  return (
    <span
...
const AuthorBlock = ({ author, role, Icon = Users }) => {
  ...
  return (
    <div className="flex items-center space-x-3 mt-6">
      ...
      <div>
        <p className="text-sm font-semibold text-gray-800 leading-tight">By {author}</p>
...
export const BlogCard = ({ post }) => {
  const { title, author, role, date, imageUrl, category, category2, color, icon: Icon } = post;
    ...
    <div className="relative overflow-hidden rounded-2xl bg-primary transition duration-300 group border border-gray-200 ">
      ...
      <div className="relative h-64 overflow-hidden">
        <img
          ...
          onError={(e) => { e.target.onerror = null; e.target.src="https://placehold.co/800x600/D0F0C0/333?text=Image+Missing"; }}
        />
...
// Component for the new Hero Section
const BlogHero = () => (
  <div 
    className="relative h-[40vh] md:h-[60vh] flex items-center justify-center text-white"
    style={{ 
      // Using a placeholder image URL for the background
      backgroundImage: `url('./images/img.jpg')`, 
      backgroundSize: 'cover', 
      backgroundPosition: 'center',
      backgroundAttachment: 'scroll' // Use 'fixed' for parallax effect if desired
    }}
  >
    ...
    <div className="relative z-10 text-center px-4 max-w-5xl">
      <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight mb-4">
        The <span className='text-secondary'>Creative</span> Marketer's Handbook
      </h1>
      <p className="text-xl sm:text-2xl font-light opacity-90 mx-auto">
        Dive into our latest research, actionable guides, and expert opinions on performance marketing and ad creative strategies.
      </p>
    </div>
  </div>
);
